...
def test_finish_rejected_on_unbacked_action_claim(make_routine, scripted):
    # D31=B / F127: a non-meta run whose ok-finish summary claims a report it never filed is
    ...
    _d, ep, status, _run_dir, events = _run(make_routine, scripted, [
        ...
        finish(summary="Steady run. Filed report to self-audit about the key gap."),
    ...
    rejected = [e for e in events if e["type"] == "observation"
                and e["payload"].get("kind") == "finish" and e["payload"].get("rejected")]
    assert len(rejected) == 1
    assert rejected[0]["payload"].get("unbacked_claims") == ["report"]
    assert "report" in ep.calls[-1]["messages"][-1]["content"]
