...
fit classifies against the engine's own window_ceiling_tokens math, so what the picker
...
EP = EndpointConfig(kind="openai", context_tokens=25_000, max_tokens=8_192)


def test_effective_window_resolves_like_the_registry():
    own = ModelConfig(endpoint="e", model="x", context_tokens=1_048_576, max_tokens=32_000)
    inherit = ModelConfig(endpoint="e", model="y")
    assert effective_window_pair(own, EP) == (1_048_576, 32_000)
    assert effective_window_pair(inherit, EP) == (25_000, 8_192)
    # no endpoint config at all → the codebase's standing defaults, never a crash
    assert effective_window_pair(inherit, None) == (25_000, 16_384)


def test_fit_classification_tracks_the_engine_ceiling():
    big = ModelConfig(endpoint="e", model="x", context_tokens=1_048_576, max_tokens=32_000)
    assert fit_fields(big, EP)["fit"] == "ok"
    # 16_384 tokens, all of it reserved for output → ceiling 0 → impossible
    dead = ModelConfig(endpoint="e", model="x", context_tokens=16_384, max_tokens=16_384)
    f = fit_fields(dead, EP)
    assert f["fit"] == "impossible" and f["input_ceiling_tokens"] == 0
    ...
    # figure agrees with the engine math in tokens
    small = ModelConfig(endpoint="e", model="x", context_tokens=25_000, max_tokens=16_384)
    f = fit_fields(small, EP)
    assert f["fit"] == "tight"
    assert 0 < f["input_ceiling_tokens"] < TIGHT_INPUT_TOKENS
    assert f["input_ceiling_tokens"] == int(window_ceiling_tokens(25_000, 16_384))
...
def test_model_window_problem_names_numbers_and_fix():
    ...
    ok = ModelConfig(name="ok", endpoint="e", model="y", context_tokens=1_048_576)
