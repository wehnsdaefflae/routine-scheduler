...
that appends to <routines_home>/.control/bug-reports.jsonl, the monotonic `R<n>` id every
report is stamped with, and the read side self-audit's gather-evidence uses. report_bug is
one of ALWAYS_KINDS — available to EVERY routine regardless of the workflow tools allowlist
...
from rsched.bug_reports import bug_reports_path, next_id
...
def test_handle_report_bug_appends_to_the_stream(tmp_path):
    ...
    assert obs == {"kind": "report_bug", "title": "gmail-body-dump perm-denied",
                   "filed": True, "id": "R1"}
    path = home / ".control" / "bug-reports.jsonl"
    assert path.is_file()
    ...
    rec = reports[0]
    assert rec["id"] == "R1"                       # the item handle the Items page indexes
...
def test_ids_are_monotonic_and_continue_an_existing_stream(tmp_path):
    """The counter lives IN the data — one past the highest id in the file — so a restored,
    hand-edited or concurrently-appended stream never reissues a number."""
    loop, home = _loop(tmp_path)
    assert [handle_report_bug(loop, {"title": f"bug {n}"})["id"] for n in range(3)] == \
        ["R1", "R2", "R3"]
    assert next_id(bug_reports_path(home)) == "R4"
    # a gap in the stream (a row removed by hand) never reissues a used number
    path = bug_reports_path(home)
    rows = [ln for ln in path.read_text().splitlines() if ln.strip()]
    path.write_text(rows[0] + "\n" + rows[2] + "\n")
    assert handle_report_bug(loop, {"title": "after the gap"})["id"] == "R4"


def test_report_bug_observation_names_the_id(tmp_path):
    """The filing run learns its item id, so it can name R7 in its own finish summary."""
    loop, _ = _loop(tmp_path)
    obs = handle_report_bug(loop, {"title": "schedule_run ate my args"})
    assert "filed as R1" in format_observation(obs)
