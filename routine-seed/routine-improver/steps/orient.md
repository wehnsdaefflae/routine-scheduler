...
2. List `~/routines` AND `~/conversations` with the `dir-tree` util (depth 1). Keep real
   routine/conversation directories only: **skip dot-directories** (`.control`,
   `.ui-traces`, …) and anything without a `routine.yaml`.
3. For each candidate, `read_file` its `routine.yaml`:
   - `improve: false` → drop it (note the skip for the LEDGER — the user opted this one
     out; never argue with it). Absent or true = included.
