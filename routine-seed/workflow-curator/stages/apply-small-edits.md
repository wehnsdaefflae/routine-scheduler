...
1. `write_file` the fix to `workflows/<slug>.py` (or the relevant trait/permission doc). Keep `META` complete, `PHASES` / `COMPLETION` present, a top-level `main()`, and every `include` resolvable.
