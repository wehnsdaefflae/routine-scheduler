...
---
# trait: global utils — your tools, and how to use them
...
If this routine holds a util-authoring permission, repair it (the CAPABILITIES section says which approval applies).
...
Creating or revising utils (`write_util`) is a separate PERMISSION: it needs a
util-authoring permission granted to this routine (the CAPABILITIES section shows whether it
is). Without it the engine rejects `write_util` — work with the existing toolbox and escalate
gaps via deferred `ask_user`.
