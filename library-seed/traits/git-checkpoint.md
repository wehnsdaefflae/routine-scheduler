...
no undo. You own that safety line. Your CAPABILITIES catalog carries a util that commits a repo
and one that restores it; run `util name=list` for their exact flags before the first call, and
record in your memory which ones you used so a later run needn't look again.
