...
- **Look for what already exists before you build.** Before standing up a store, a site, a
  pipeline — any reusable artefact — go and check whether one is already there: the filesystem,
  neighbouring projects, the configuration, your own notes. A second system doing an existing
  system's job is the most expensive kind of scope creep, because both then need maintaining
  and somebody has to work out which one is real.
