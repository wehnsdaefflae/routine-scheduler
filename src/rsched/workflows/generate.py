...
META = {
    ...
    "includes": [<rule slugs from the available list — the general rules this pattern \
suggests>],
...
def generate(server: ServerConfig, instruction: str, hint: str = "",
    ...
    prompt = (
        ...
        + (f"SHAPE HINT: {hint}\n\n" if hint else "")
        + f"{FORMAT_SPEC}\n\nAvailable rules for `includes`: {rules}\n\n"
        ...
        "Write INSTRUCTIONS, NEVER HISTORY: the agent that reads this pattern has never seen an "
        "earlier draft of it, so state the current design in the present tense and stop. No "
        "'no longer', 'used to', 'previously', 'was retired/replaced', 'never reintroduce', "
        "'the old <thing>', and no dates or counts that were true once. "
    ...
    for attempt in range(2):
        ...
        problems = lint_workflow_py(draft, filename=f"{slug}.py", rule_slugs=rules)
        if not problems:
