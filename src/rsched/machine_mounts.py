"""sshfs SHARE MOUNTS — the filesystem half of a bound machine.

Split out of `machines.py` (F393). Resolving a machine binding into env vars is config
plumbing; mounting a remote filesystem into the run's directory is a privileged, stateful
operation with its own lifecycle, and it is the half that can lose data.

The engine (unsandboxed) does the mount so the private key never enters a util, and a mount is
PROVEN live before it is advertised: sshfs daemonizes, so exit 0 means the helper forked, and
the mountpoint is created BEFORE the mount, so a failure would otherwise leave an empty
directory reading exactly like an empty share (R514).
"""
