...
makes a group fire its members BACK-TO-BACK: fire member 0, wait for it to reach a TERMINAL
state, then — per its outcome and the group's on_failure policy — fire member 1, and so on.
...
group: either "the in-flight member is still running → wait" or "it terminated → record it,
apply the policy, fire the next (or finish)". Nothing fires two members of one group at once.
...
class GroupRunManager:
    ...
    def _collect(self, rec: dict, catalog: dict[str, registry.RoutineInfo]) -> None:
        ...
        elif int(rec.get("cursor") or 0) >= len(rec.get("members") or []):
    ...
    async def _fire_next(self, rec: dict, catalog: dict[str, registry.RoutineInfo]) -> None:
        """No member in flight: fire the member at the cursor, or finish the chain. The daemon
        owns the drain + one-run-per-routine rules, exactly as for cron/trigger/one-shot fires.
        """
        gid = str(rec.get("group_id") or "")
        ...
        if info is None or not info.cfg.enabled:
            rec["log"].append({"slug": slug, "run_id": None,
                               "state": "skipped", "outcome": "failed"})
            rec["cursor"] = cursor + 1
        ...
        if rid:
            ...
            log.info("group fired member group=%s slug=%s run=%s (%d/%d)",
                     gid, slug, rid, cursor + 1, len(members))
