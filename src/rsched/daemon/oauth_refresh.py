...
at connect. The async `tick` runs it in a worker thread so it never blocks the scheduler loop.
Store writes are serialized by the store's own lock, so the worker thread and the web callback
