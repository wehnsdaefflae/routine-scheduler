...
def restart_action(requested: bool, active_states: list[str], draining: bool) -> str:
    """Pure state machine. Returns one of:
      ...
      'drain'   — request pending, only finishable runs active: start draining (fire nothing new)
      'restart' — draining and nothing left active: exit so the supervisor relaunches new code
    """
    if not requested:
        return "idle"
