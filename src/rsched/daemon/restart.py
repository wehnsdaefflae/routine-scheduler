...
The daemon notices it, DRAINS (stops firing new runs and waits for the active runs to finish),
...
def restart_action(requested: bool, active_states: list[str], draining: bool) -> str:
    ...
    'drain'   — request pending, only finishable work active (engine runs): start
                draining — fire nothing new
    'restart' — draining and nothing left active (no runs): exit so the supervisor
                relaunches new code
    """
    if not requested:
        return "idle"
