...
- `OBSERVATION (finish deferred): your summary does not account for the open STOPPING CONDITIONS s1, s2 …` — F334/D98 v1, the ACCOUNTING rung: a depth-0 finish must carry a `[s<n>] met — <evidence>` / `[s<n>] unmet — <what remains>` line per ACTIVE condition (`engine/stopping.unaccounted`). The engine checks the MENTION, never the semantics — a dormant condition is never demanded, and the spent reserved-finish turn is exempt because deferring it would force-finish the run with an engine string.
- `OBSERVATION (finish deferred): you reported s1 unmet without saying what REMAINS…` — v1b, the RESIDUAL rung (`engine/stopping.without_residual`): v1 proves the condition was addressed, this proves an `unmet` verdict carries its residual. A run bound never transitions, so that note is the whole of what the next run inherits about it — a bare `[s1] unmet` clears v1, stores an empty note and hands the next run a verdict with no content. Deterministic like the rung above: emptiness is checkable, adequacy is the model's business.
- `OBSERVATION (finish deferred): a general rule you practise applies to how this run ends.` + one `[RULE <slug> — <what fired>] <line>` per assist + `Act on it and finish again — this is asked once per run, so the next finish stands either way.` — a `pre-finish` RULE ASSIST (docs/rule-assists.md). The only assist moment that costs a turn — and it has to: a line surfaced as the run ends is one nobody can act on. It carries the same two guards as the rungs around it (never a child run, never the reserved finish turn) plus its own — a run is held at its finish by an assist at most ONCE, ever, so a rule may ask for an ending to be reconsidered but never negotiate over it.
