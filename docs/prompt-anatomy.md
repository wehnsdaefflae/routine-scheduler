...
- `OBSERVATION (procedure poll-inbox, exit 0):\n<stdout>` — the `procedure` action (D88, gated by the `procedure` capability): the routine's OWN `procedures/<name>.py` script, run in the routine's workdir venv with the routine's own filesystem permissions (recipe and procedure see the same files), rendered with the same body (`[stderr]`, `[full output]` spill pointer, the F290 withheld-optional `[note]`). A missing script teaches the authoring route (`write_file procedures/<name>.py` + the docstring header); the same four-state secret-exposure gate runs before it, its wording saying "procedure" where the util gate says "util"
