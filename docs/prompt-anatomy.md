...
- `OBSERVATION (procedure poll-inbox, exit 0):\n<stdout>` — the `procedure` action (D88, gated by the `procedure` capability): the routine's OWN `procedures/<name>.py` script, run in the routine's workdir venv under the SYMMETRY rule — the run's fs roots (recipe and procedure see the same files), the routine's STANDING settings as env (granted secrets, connections, machines), the util library on PATH (`gu llm` asks the routine's own default model), rendered with the same body (`[stderr]`, `[full output]` spill pointer, the F290 withheld-optional `[note]`). A missing script teaches the authoring route (`write_file procedures/<name>.py` + the docstring header); the same four-state secret-exposure gate runs before it, its wording saying "procedure" where the util gate says "util"
