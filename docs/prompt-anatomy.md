...
plus the always-available `finish` / `report_bug`. A kind the engine would reject gets no
...
| 9 | `# WORK ORDERS FROM OTHER ROUTINES (consume now)` | the same inbox drain | The drained messages a SIBLING ROUTINE filed with `hand_off`, split out of (8): a work order is not something the user said, and rendering it as one would have the run answer the wrong party. Each carries its own `WORK ORDER <id> from routine <slug>` heading. Mid-run they arrive as `WORK ORDER (injected mid-run)` (§2). How to receive one is the `work-orders` permission doc's job in (6), so this section carries no standing prose. |
...
- `OBSERVATION (hand_off 'routine-improver': filed as W3 — delivered to its inbox; that routine reads it on its next scheduled run).` / `…no routine 'x'…` (with `suggestions` + `valid_targets`, like schedule_run) / `…self_target…` (a note to yourself is `note` or `memory_write`) / `…filed: false…` (the ledger write failed — say so in the finish summary; unlike report_bug this is NOT best-effort, because a run that believes it routed work will not route it again). `interact.handle_hand_off` writes the `W<n>` ledger row + the target's inbox message un-sandboxed; the target's own drain stamps delivery back onto the row (`rsched/work_orders.py`).
- `OBSERVATION (report_bug filed as R7: 'schedule_run ate my args' — appended to .control/bug-reports.jsonl; the self-audit routine will review it. Refer to it by that id if you mention it again. Continue your own task.)` — `R7` is the id the report is stamped with as it is appended (`rsched/bug_reports.py`), and the item's handle on the console's Items page (docs/items.md); the failure branch says the report was NOT filed and to mention the bug in the finish summary instead
...
- hand_off: send a WORK ORDER to another routine — `target` (its slug), `title` + `detail` (the artefact, what is wrong, the evidence, what "done" looks like). It is filed under a `W<n>` id in `.control/work-orders.jsonl` and lands in that routine's inbox; that routine reads it on its NEXT SCHEDULED RUN. Nothing is started and nobody is interrupted. `answers: "<W id>"` closes a work order this routine RECEIVED. Gated by the **work-orders** capability — a delivered work order becomes prose in another routine's prompt, so an ungated version would be a cross-routine injection channel.
- report_bug: file a bug report about the SCHEDULER itself (engine, a util's CLI, the web UI, a workflow) — a defect or friction hit while running. `title` + optional `detail`; appends to a shared bug stream the self-audit routine reads every run and turns into findings (it does not interrupt anyone or reach the user). Available to EVERY routine by default (ungated, one of `ALWAYS_KINDS` alongside `finish`).
