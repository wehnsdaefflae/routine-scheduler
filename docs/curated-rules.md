# The curated rules — provenance and evidence

The **curated rules** are the ones in the provenance table below: this file's table IS the list,
and a rule of the shipped set ([rules & permissions](rules-permissions.md)) is curated exactly
when it has a row here. They are distilled from external prompt-engineering guidance rather than
from this project's own experience — `status-page` is the one exception, written from this
instance's own record and kept here because the same growth rule governs it. This file records
...
composed prompt, toggleable per routine. It was rejected in favour of ordinary **rules**, for
...
| `test-design` | [`pr-review-toolkit/agents/pr-test-analyzer`](https://github.com/anthropics/claude-code/tree/main/plugins/pr-review-toolkit) | No effectiveness data, but it closes a hole **underneath** `independent-verification`: that rule says "prefer a mechanical check", and a test that asserts the implementation just written is a mechanical check that cannot fail. The hollow check then reports a pass and the verification rule amplifies it. "Watch it fail once" is executable in this engine (run it, observe a nonzero exit) rather than aspirational. |
| `failure-visibility` | [`pr-review-toolkit/agents/silent-failure-hunter`](https://github.com/anthropics/claude-code/tree/main/plugins/pr-review-toolkit) | No effectiveness data. Kept for one bullet that is unusually good because it is *checkable*: before writing a broad catch, enumerate which unrelated failures it will now absorb. Note the deliberate split from `error-recovery` — that rule is the agent reacting to its own failed observation, this one is the error handling the agent writes into code. The closing bullet is load-bearing: without it the rule pulls against `change-restraint` and produces defensively over-wrapped code. |
| `status-page` | this instance's own store of web-UI feedback (~65 rows across five project stores, 2026-07-24 → 2026-08-28) | **Strongest evidence in the set, and the only rule here written from observed failure rather than from an upstream source.** Every clause answers a complaint that is on the record, most of them repeated across unconnected projects: a read-only draft ("Why didn't you make the text field editable?!"), a question offering only Agree/Disagree ("that question cannot be answered with 'Agree' or 'Disagree'"), feedback that could not be revised before it was consumed (asked on ards seq 6 and 9 AND nanogeofeld seq 9), already-answered feedback still listed ("why do i still see *ALL* previous feedback?"), and generated documents not checkable in the UI (a standing instruction, ards seq 17). The two payload invariants it insists on — `feedback_cursor` and `documents` — had each already regressed in production by a run rebuilding its payload from scratch. Three clauses added 2026-09-02 come from REPORTS rather than from feedback rows, each named after the fault it prevents: the `WEB_AUTH_SOURCES` grant a publisher must hold before its first publish, with `gate.php?diag` as the way to tell a missing credential from a header the server ate (R1143, R1175 — one steward lost runs while every part of its publish path was correct; granting the secret fixed it on the first attempt); that `gate-file.php?p=` resolves against the hub DOCROOT, so a routine linking a path on its own disk publishes a 404 on a document the page says is ready (R1199); and that the shared kit at `<libraries_home>/web/steward/` has to be read to build against it, which every routine had been working around by writing itself a private helper script (R1160). That last one is also the rule's only `expects:` entry — `fs-read` on that directory — so the setup surface reports the missing root on every holder instead of each routine discovering it alone; an `fs-write: ["*"]` row rode beside it until it was checked against all seven holders and dropped, because a page is published through an upload channel and a routine's own dir is always writable. Four instructions added 2026-09-22 answer filed reports the same way: the shell's steer box takes the publisher's own `label`/`placeholder`, so the box asks the project's question instead of the generic one and nobody builds a second one above it (R1821); a question may mark at most one option `recommended`, which leads the row and pre-fills his box rather than sending for him (R1793, R1779); `put-state` answers with `emptied`, naming every section that had entries last time and has none now — an unintended one is a failed publish, because an empty section renders as nothing at all and a page once lost its whole template shelf under a publish that verified itself clean (R1822); and one editor at a time is the HOST's courtesy between readers, expiring on its own and never touching a routine, which publishes whole documents between runs rather than keystrokes (R1819, R1360). Beside them one statement of fact: the page stamps a control id with the project slug as it files, so a routine names a control whatever reads well and matches on the prefix when it reads the store back (R1800). |
| `risk-first` | [bitkentech/shipsmooth](https://github.com/bitkentech/shipsmooth)'s risk-quality control strategy (Apache 2.0), itself an application of Boehm's spiral model | **Design intent only, no effectiveness data**, and the source is a small single-author project — so it is carried on the argument, not on the author. The argument is the part that holds: the probability the APPROACH is wrong peaks before anything runs and collapses once a thin path works end to end, while quality only starts to matter once the approach will survive — so work spent on polish before that point is paid for twice. It counters a tendency rather than restating knowledge (models harden as they go, and they take a plan's reading order for its risk order), which is the bar this file sets. Checked for collision with the three rules nearest it: `decision-commitment` says when to stop deciding, this says which work to take first; `change-restraint` says build the smallest thing, this says build the least certain thing first; `test-design` is unaffected, because the hardening half is explicitly non-optional and expires the licence to skip it. The last two bullets exist to stop the rule being read as permission to ship scaffolding — the failure mode a de-risking rule has if it is written carelessly. |
...
| The `code-reviewer` agents' confidence gating (`pr-review-toolkit`, `feature-dev`) | Their headline instruction is "only report issues with confidence ≥ 80", scored 0–100. That **directly contradicts `review-recall`** ("find first, filter second"; "uncertainty is a label, not grounds for omission") and re-introduces the numeric-confidence pattern already rejected above. A rule that fights another shipped rule is worse than no rule. Only one idea survived, as a single `review-recall` bullet: separate what the change introduced from what it inherited. |
| `type-design-analyzer` | Four 1–10 ratings — the numeric pattern again. The underlying content ("make illegal states unrepresentable", constructor validation, immutability) is textbook material the model already applies; a rule's job is countering a tendency, not restating knowledge. |
...
- **`comment-rot`** (from `pr-review-toolkit/agents/comment-analyzer`) — the strongest held candidate,
  ...
  has yet produced the failure, and three code-facing rules landing at once is more than the
...
- **`risk-first`** shipped instead of being held, and the reason is worth recording: unlike the
  three below it duplicates nothing already in the set (the collision check is in its provenance
  row), and its claim is structural rather than empirical — it is about the ORDER work is done
  in, which no other rule speaks to. It ships UNBOUND like every curated rule, so the first
  routine to hold it is a decision somebody makes, and `run_health.py` buckets the runs either
  side of that recipe change. That is the evidence this file asks for, gathered the way it says
  to gather it.
- **`codebase-orientation`** (from `feature-dev/agents/code-explorer`) — trace one execution path end
  ...
  looking) and `decision-record` (record it so the next run doesn't re-explore) already cover, the
...
wholesale: **start minimal and add instructions only in response to observed failures.** A rule
added because it sounds wise is indistinguishable, at write time, from one that works.

This instance can do better than taste, because a rule is named in a routine's recipe and
